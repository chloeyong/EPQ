...
'Handle log movement
	Local r: Float = Rnd(1,7)'chooses a random number inbetween 1 and 4
...
'All the graphics drawing goes here:
	Select GameState
		...
		Case "PLAYING"
			...
			DrawText(speed, 100, 60, 2, 2)
			'Local r: Float = Rnd(1,4)'chooses a random number inbetween 1 and 4
			For Local enemy:= Eachin enemy_collection
				DrawImage enemy.sprite, enemy.x, enemy.y 'draws the log
				If speed < 1.0 Then enemy.y += 1 'moves the enemy down the y-axis
				If speed > 1.0 Then enemy.y += 4
			End
			For Local worm:=Eachin worm_collection 
				DrawImage worm.sprite, worm.x, worm.y 'draws the log
				If speed < 1.0 Then worm.y += 1
				If speed > 1.0 Then worm.y += 3
			End
		End	
