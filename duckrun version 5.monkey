...
'Handle log movement
	Local r: Float = Rnd(1,4)'chooses a random number inbetween 1 and 4
	For Local enemy:=Eachin enemy_collection
		If enemy.y>480-16 Then 'creates a condition to remove the log once they have reached the bottom of the screen
			...
			End
			If PlayState = 3
				enemy_collection.AddLast(New log(205,0)) 'and add new logs and worms
				enemy_collection.AddLast(New log(320,0))
				worm_collection.AddLast(New worm(90,0))			
			End
		End
	End
	For Local enemy:=Eachin enemy_collection
	For Local worm:=Eachin worm_collection 'creates a condition if the player collides with the worm
		If intersects(player.x,player.y,10,21,worm.x,worm.y,100,50) Then
			...
			score = score + 1 'adds one to the score
				'If score > 5 Then movementspeed = 500
		End
	End
	End 
	...
	For Local worm:=Eachin worm_collection
		If intersects(player.x,player.y,10,21,enemy.x,enemy.y,100,50) Then 
			...
			If highscore > score Then highscore = highscore
			endgame = -1 'sets score to = -1 (so the game ends)
		End
	End
	End
End
...
'All the graphics drawing goes here:
	Select GameState
		...
		Case "GAMEOVER"
			...
			DrawText(score, 510, 370, 10, 10)	
			DrawText(highscore, 510, 400, 10, 10)
		Case "PLAYING" '"PLAYINGLEVEL1"
		...
		'	For Local enemy:= Eachin enemy_collection
		'		DrawImage enemy.sprite, enemy.x, enemy.y 'draws the log
		'		enemy.y += 2 'moves the enemy down the y-axis
		'	End
		'	For Local worm:=Eachin worm_collection 
		'		DrawImage worm.sprite, worm.x, worm.y 'draws the worm
		'		worm.y += 2 'moves the worm down the y-axis
		'	End
		'Case "PLAYINGLEVEL3"
		'	DrawImage background, 0, 0 'draws the background picture as a background
		'	DrawImage player.sprite, player.x, player.y 'draws the player sprite
		...
		'	For Local enemy:= Eachin enemy_collection
		'		DrawImage enemy.sprite, enemy.x, enemy.y 'draws the log
		'		enemy.y += 4 'moves the enemy down the y-axis
		'	End
		'	For Local worm:=Eachin worm_collection 
		'		DrawImage worm.sprite, worm.x, worm.y 'draws the worm
		'		worm.y += 4 'moves the worm down the y-axis
		'	End
		End
		'If score < 5 Then GameState = "PLAYINGLEVEL1"
		'If score > 10 Then GameState = "PLAYINGLEVEL2"
		'If score > 15 Then GameState = "PLAYINGLEVE3"
		If endgame = -1 Then GameState = "GAMEOVER" 'condition - if the score is less than 0 then the game ends
	End
End		
...
Class log 'creates a new class for the alien
	...
	Method Move(y_distance:Int) 'this indicates the y moves down the axis
		y+=y_distance
	End
	'Method Speed(x_speed:Int, y_speed:Int)
	'	bdX = x_speed
	'	bdY = y_speed		
	'End
End
